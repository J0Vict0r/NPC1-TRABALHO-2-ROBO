...
public class SegundaMain {
    public static void main(String[] args) throws InterruptedException {
        ...
        System.out.println("-------BEM-VINDO AO JOGO RANDÔMICO COM DOIS ROBÔS PADRÕES-------");
        ...
        while (!encontrouAlimento) {
            ...
            System.out.println("Vez do Robô 2 (" + cor2 + ") jogar.");
            int movimento2 = random.nextInt(4) + 1; // Movimento aleatório para o robô 2

            try {
                robo2.mover(movimento2);
                if (robo2.getRoboX() < 0 || robo2.getRoboY() < 0 || robo2.getRoboX() > tamanhoX || robo2.getRoboY() > tamanhoY) {
                    throw new MovimentoInvalidoException("Robô 2 tentou sair do plano!");
                }
            } catch (MovimentoInvalidoException | ArrayIndexOutOfBoundsException e) {
                System.out.println("Robô 2 tentou sair do plano!");
                robo2.setRoboX(Math.max(0, Math.min(robo2.getRoboX(), tamanhoX)));
                robo2.setRoboY(Math.max(0, Math.min(robo2.getRoboY(), tamanhoY)));
            }

            // Exibe a área após o movimento do robô 2
            exibirArea(area, robo1.getRoboX(), robo1.getRoboY(), robo2.getRoboX(), robo2.getRoboY(), alimentoX, alimentoY, tamanhoX, tamanhoY, cor1, cor2);
            Thread.sleep(1000); // Pausa de 1 segundo
            System.out.println("-------------------------------------");
            ...
            if (robo1.verificarPosicaoAlimento(alimentoX, alimentoY)) {
                ...
                System.out.println("Robô 2 (" + cor2 + ") encontrou o alimento!");
                encontrouAlimento = true;
            }
        }
        ...
        // Finaliza o jogo
        System.out.println("----------------------------------");
        System.out.println("ESTATÍSTICAS - ROBÔ 1 "+ "("+robo1.getCor()+")" +": ");
        ...
        System.out.println("------------------------------------");
        System.out.println("ESTATÍSTICAS - ROBÔ 2 " + "("+robo2.getCor()+")" + ": " );
    ...
    public static void exibirArea(int[][] area, int robo1X, int robo1Y, int robo2X, int robo2Y, int alimentoX, int alimentoY, int tamanhoX, int tamanhoY, String cor1, String cor2) {
        ...
        area[robo2Y][robo2X] = 2; // 2 representa o robô 2
        ...
        for (int i = tamanhoY; i >= 0; i--) { // Começa do topo (maior Y) até 0
            for (int j = 0; j <= tamanhoX; j++) {
                if (area[i][j] == 1) {
                    System.out.print("R1" + " "); // Exibe o robô 1
                } else if (area[i][j] == 2) {
                    System.out.print("R2" + " "); // Exibe o robô 2
                } else if (area[i][j] == 3) {
